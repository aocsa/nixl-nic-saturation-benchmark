#!/usr/bin/env python3
"""DRAM UCX NIXL saturation harness for DGX Spark CX-7 twins.

Control plane is HTTP rendezvous on mgmt (agent metadata + xfer descs).
Data plane is NIXL UCX RoCE. Matches nixlbench DRAM WRITE pairwise /
manytoone / onetomany.
"""
...
def wait_notif(agent, peer, tag: bytes, timeout_s=180.0):
    t0 = time.time()
    while True:
        notifs = agent.get_new_notifs()
        if peer in notifs:
            for msg in notifs[peer]:
                if tag in msg:
                    return msg
        if time.time() - t0 > timeout_s:
            raise TimeoutError(f"no notif {tag!r} from {peer}")
        time.sleep(0.005)
...
def make_agent(name: str, listen_port: int):
    config = nixl_agent_config(
        True, True, listen_port, backends=["UCX"], num_threads=4
    )
    agent = nixl_agent(name, config)
    log(f"agent {name} listen={listen_port} host={socket.gethostname()}")
    return agent
...
def run_initiator(args, peers: list[tuple[str, str, int]]):
    ...
    rv_put(f"/k/{args.group}/ready/{args.name}", b"1")
    # wait until every peer published ready (target registered + loaded us)
...
def main():
    ...
    log(
        ...
        f"group={args.group} rail={args.rail} "
